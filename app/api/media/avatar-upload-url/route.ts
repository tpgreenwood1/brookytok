...
export async function POST(req: NextRequest) {
  ...
  if (!isImageType(contentType)) {
    return NextResponse.json(
      { error: "Avatars must be an image (JPEG, PNG, WebP, or GIF)" },
      { status: 422 }
    );
  }
  ...
  const objectKey = generateObjectKey("media/avatars", session.user.id);
  ...
  try {
    ...
    console.error("[avatar-upload-url] Failed to generate presigned URL:", err);
    return NextResponse.json(
  ...
  return NextResponse.json({ uploadUrl, objectKey, avatarUrl });
}
